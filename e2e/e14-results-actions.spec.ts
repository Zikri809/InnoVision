...
test.describe("E14 — results action buttons", () => {
  test("Unlock button; Face-exempt dialog; Reset confirm-cancel", async ({ browser }, testInfo) => {
    ...
    const exemptBtn = lecturerPage.getByRole("dialog").getByRole("button", { name: "Face-exempt", exact: true });
    ...
    await lecturerPage.getByRole("dialog").getByRole("button", { name: "Cancel", exact: true }).click();
    await expect(
      lecturerPage.getByRole("dialog"),
    ).toBeHidden();
