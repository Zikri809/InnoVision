...
/**
 * Mobile-only answer-commit capture size (prod 2026-09-26 perf fix).
 * Applied when `isCoarsePointerDevice()` (phone/tablet): a 320px source is
 * still ~3× oversampled for the 112×112 embedding input — zero accuracy
 * impact — while cutting per-answer upload ~4–6×. Desktop webcams are
 * optically limited, so desktop keeps full-res answer captures; enroll +
 * periodic verify keep full resolution on every device (rare, and enroll
 * feeds the stored baseline where every pixel of margin counts).
 */
