...
|---|---|---|
| **Extraction location** | **Client-first, server-fallback.** The cascade (`lib/extract/pipeline.ts`) runs in the **lecturer's browser** (Tesseract WASM, GLM via local Docker/vLLM, vision via `/api/ocr/vision`). `/api/ai/generate-quiz` accepts `extractedText` and uses it directly; if absent it runs a **server-side native text parse** of the stored file (I16b), **bounded and hardened** (see S1). Server never renders PDF pages for OCR. | Matches PLAN §3 (client OCR, $0, no serverless burden) while honoring the §2 "else server-side native parse" contract. |
| **AI client** | `openai` npm SDK with `baseURL` override, **server-only** (`lib/ai/client.ts` reads `AI_BASE_URL`/`AI_API_KEY`/`AI_MODEL`; `import "server-only"` at top). GLM-OCR (browser→localhost vLLM) uses a **plain-fetch OpenAI-compatible helper** (`lib/ai/http-compat.ts`, browser-only) — no SDK in the browser, no key needed. | PLAN locked decision (§0). Keeps the cloud key server-side; local vLLM needs no key and no SDK. |
...
- `src/lib/extract/glm-ocr.ts` — `probeGlm({ baseUrl })` → `GET {baseUrl}/v1/models` (timeout ~2s; **client-only** — server never proxies localhost, closing SSRF); `extractWithGlm(file, { baseUrl, model })` → render pages (pdf.js) → `POST {baseUrl}/v1/chat/completions` via `http-compat.ts`.
...
| SSRF | GLM probe/chat are **client-only** (lecturer's own machine); server never proxies `GLM_BASE_URL`; vision route ignores body `baseUrl` (I-A14, S8). |
...
- **60s + local vLLM generation:** slow local models may still exceed 60s if a lecturer picks server-side native parse on a big file — mitigated by client-first extraction + text cap + 45s wrapper; route returns 503 `timeout` cleanly.
