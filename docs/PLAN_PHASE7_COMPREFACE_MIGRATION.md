...
| L15 | **E2E CompreFace mock: frame-marker inspection (NOT a client global)** | The E2E fake tracker returns deterministic frame marker strings: `"data:image/jpeg;base64,FAKE_FRAME_MATCH"` or `"data:image/jpeg;base64,FAKE_FRAME_MISMATCH"`. The Next.js route inspects the frame string (when `process.env.NEXT_PUBLIC_E2E_FAKE_SEAM === '1'` AND `process.env.COMPREFACE_MOCK_ENABLED === '1'`) and returns a canned CompreFace response without calling Docker. `setVerifyMode('match'|'mismatch')` controls which marker the fake tracker returns. **Two-flag guard** (`NEXT_PUBLIC_E2E_FAKE_SEAM` + `COMPREFACE_MOCK_ENABLED`) prevents production leakage. The `__INNOVISION_FAKE_COMPREFACE__` client global is NOT used (routes are server-side — they cannot read browser globals). | **CRITICAL:** the route is server-side; a client global is architecturally impossible. The frame-marker mechanism is the transport. The `COMPREFACE_MOCK_ENABLED` env is set in `playwright.config.ts` and by the route tests' `beforeAll`. *Gate update (2026-08-29):* the first flag was `NODE_ENV !== 'production'`; since 5f6b1da the suite serves the production build (where that gate is dead), so `NEXT_PUBLIC_E2E_FAKE_SEAM` (`seam-gate.ts`) replaced it. |
...
- **`src/lib/face/server/compreface-client.ts`** (NEW, server-only):
  ...
  - **Mock mode:** when `process.env.NEXT_PUBLIC_E2E_FAKE_SEAM === '1'` AND `process.env.COMPREFACE_MOCK_ENABLED === '1'` (`isMockMode()` in `compreface-client.ts`), the client inspects the frame string for marker substrings (`FAKE_FRAME_MATCH`, `FAKE_FRAME_MISMATCH`) and returns canned responses without calling Docker. (First flag was `NODE_ENV !== 'production'` — dead under the production-build E2E harness since 5f6b1da.)
...
| E2E mock leaks to production | Two-flag guard (`NEXT_PUBLIC_E2E_FAKE_SEAM === '1'` AND `COMPREFACE_MOCK_ENABLED === '1'`); the harness flag replaced the original `NODE_ENV !== 'production'` co-gate, which was dead under the production-build E2E harness (5f6b1da). | **NEW**. |
