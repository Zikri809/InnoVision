...
| `0062_quiz_integrity_gates.sql` | I-27/I-28/I-29/I-30 (audit-5 M1–M4): assessment start without consent → `consent_required` and pending_review → `face_enrollment_pending` (practice unaffected); `enroll_face` with a live assessment session → `live_assessment`; `submit_session` writes ONE dedupe-guarded `auto_flag_verify_silence` marker (`via:'submit'`) for a suppressed attempt while an honest submit records none; `approve_face_enrollment` flips pending_review → enrolled (audited) and refuses a non-pending enrollment (`not_pending`); the `session_verify_silent` helper contract | 13 |
...
| E5b | **Lecturer resets attempt** | active attempt (not submitted) → lecturer deletes session from results → student restarts → mid-flight reset dead-screens | student can start fresh; reset audited; **audit-2 M-07**: resetting a `completed` attempt is REFUSED (409 `session_not_active`, row survives, nothing audited — a graded record is append-only) |
...
| E18 | **Results Excel export (0027)** | lecturer: class + 2-question assessment + publish; student completes with one right/one wrong → results dashboard → **Question insights** (a LINK to its own `/insights` route since RA-2 — the on-screen model mirrors the workbook) → **Export Excel** button captures the real BLOB download → file parsed with exceljs | suggested filename `<title>-results-YYYY-MM-DD.xlsx`; exactly Results / Questions & Key / Choice Distribution sheets; student row carries matric + name + Completed + score `1/2` + numeric `50%` + per-question cells (`B — 4` green, wrong red); key letters B/A with per-question stats; distribution rows for both questions. **Column positions are derived from the header row**, never hardcoded: the assessment layout carries four integrity columns (face fails / focus pauses / fullscreen exits / hand pauses) between Duration and Q1, so a literal index rots when that set changes. Also the only UI-level guard that the REQUIRED matric registration field (0027) doesn't strand the flow (`registerUser` helper fills a deterministic 6-digit matric and returns it). **Zero-session leg**: a joined-but-never-attempted student still gets a roster row with Status "Not started" and an empty (null) score cell — headers intact. |
| E26 | **Class archiving lifecycle (HIGH #9)** | 3 serial tests: (1) lecturer archives a live class with an enrolled student (confirm dialog → `/lecturer/classes/archived`; the `#archive-search` filter isolates the card); (2) student loses visibility — class gone from `/student/classes`, quiz gone from `/student/quizzes`, rejoin attempt hits the neutral "That join code is not valid." alert (**audit-2 M-04**: `class_archived` is folded into `invalid_code` route-side so an archived class is indistinguishable from an unknown code — no existence oracle); (3) restore re-exposes class + quiz (banner "This class is archived" asserted pre-restore) | archive lands on the archived page; student views exclude the class and quiz; `join_class` rejects; restore flips everything back |
...
- `LECTURER_INVITE_CODE` MUST be set (env or `.env.local`) — both e51 tests `test.skip` without it, so a run reporting "skipped (2)" means the code was missing, not that the hardening passed.
- Make sure `.env.local` does NOT set `NEXT_PUBLIC_INTEGRITY_HARDENING_OFF=1`: playwright.config.ts loads it with `override: false`, so a leaked value forwards into the build and bakes the kill switch even under `INTEGRITY_E2E=1` (the assertions then fail loudly, but the run is wasted).

The env var does double duty: e51's own `test.skip` gate keys on it, and the webServer build omits the kill switch only when it is set. A production build that bakes the kill switch by accident warns once in the console (`[integrity-gate] …`, `hardening-gate.ts`) — CI's main job never trips it because the harness seam flag also suppresses the warn.

### 5.3 E2E debugging playbook — lessons from the 2026-08-26 mass-failure repair
...
15. **The EndScreen mounts BOTH compositions at once (2026-09-15 mass repair, ~12 specs).** `components/quiz/end-screen.tsx` renders a mobile layout (`lg:hidden`) and a wide layout (`hidden lg:block`) in the SAME commit — so any text that appears in both (the banner "Practice complete! 🎉" / "Assessment complete! 🎉", the score "1 / 2" as a ScoreRing label+sub pair vs. a single `<p>`, "100% correct", "Submitted ✓ — results will be released by your lecturer.") resolves to **2 elements** and strict-mode-violates. The failure signature is a `strict mode violation: getByText(...) resolved to 2 elements` whose two entries differ only in class names (`text-xs` vs `text-sm`, `relative` present/absent). Fix: scope to the visible one — `locator("p:visible", { hasText: ... })` or `locator(":visible", { hasText: /^1\s*\/\s*2$/ }).first()` (e36/e42 precedent). Note `sr-only` is NOT invisible to Playwright (it keeps a 1px box), so live-region twins (the play client's polite announcer carrying `keepHandVisible`, e9b) also need `span:visible` / a role-scoped locator rather than a bare text match. Do NOT "fix" this by deleting one composition.
16. **Focus-loss strikes are deduped for 2s — `focus_pause_count` can stall (e16).** `use-face-pipeline.focusLossPause` consults a shared pause stamp (`FULLSCREEN_PAUSE_DEDUPE_MS = 2000`, `use-fullscreen-guard.ts`) that collapses the fullscreen-exit + blur double-fire of a SINGLE app switch into one strike. The deduped branch still flips the LOCAL overlay to paused **without POSTing**, so "the overlay appeared" does NOT prove the server counted the strike. A spec that dispatches synthetic `focus`/`blur` events back-to-back gets its later strikes swallowed (DB shows `focus_pause_count` stuck at 1–2, status `paused`, never `flagged`). Fix: gate each simulated strike on the `/pause` POST **response** and assert the returned `sessionStatus`, retrying when a strike was deduped (e16 pattern) — never loop on overlay visibility alone.
17. **Transient streaming nodes race the assertion (e2d).** The generation console's grey one-liner renders only while `phase === "running"` (`GenerationProgress.tsx`), so it is REMOVED at the terminal morph and a direct `toBeVisible()`/`toHaveText()` races the stream (observed: "element(s) not found" while the payoff was already on screen, or a late stage line like `Save — skipped` that a narrow regex misses). Fix: install a `MutationObserver` (or `page.on("request")`) BEFORE the action under test and assert on the recorded history, not on a live snapshot.
18. **Full-suite load inflates mock-stream latency (e2f flags).** The AI/TinyFish mock servers are plain node processes shared by every worker; under 6 workers a stream that finishes in ~2s solo can exceed a 30s assertion budget (symptom: timeout while the dialog still shows "Drafting questions…", and the spec passes 3/3 in isolation). Give terminal payoff assertions the same headroom as their sibling specs (`90_000`–`120_000`), and only after ruling out a real hang.
19. **Reporters: `onBegin(config, suite)` — the suite is the SECOND argument.** `scripts/e2e-min-exec-reporter.mjs` read it from the first parameter, so `this.suite` was the config object, `suite.allTests()` threw a TypeError, and EVERY run (green or red) ended with `TypeError: suite.allTests is not a function`. If the reporter output is missing its `[min-exec] N/M tests executed` line, check this signature first.
...
8. **Grounded web search pre-demo gate** (TinyFish topic mode): `TINYFISH_API_KEY=... node scripts/eval-grounded-quiz.mjs --runs 1 --topics t1,t3` for a quick smoke (~5 min), and the full gate `--runs 3` before demo day (~20–40 min). Also run `node scripts/spike-tinyfish.mjs` once per new key to confirm the endpoint shape. If TinyFish is down or the key is missing, demo fallback: use the file/paste flow (untouched by the feature) — the "Web topic" mode simply hides.
