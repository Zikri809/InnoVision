...
| L6 | Pose gates | **SUPERSEDED (prod 2026-09-21)** by `lib/face/pose-gate.ts`: shared per-angle bands (front \|yaw\|≤15, sides 10–45, neutral-relative), judged against the client's accepted `yawReading`; the sidecar's absolute yaw is an anti-tamper bound (≤75°). Legacy/no-reading fallback keeps the original absolute bands: front \|yaw\| ≤ 30°; sides 10–75°; skipped in mock mode (mock yaw = 0 would fail sides) | route-side + capture-side, on the selected primary face |
