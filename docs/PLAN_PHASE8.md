...
- **Service-role seam in E2E.** `resolveServiceClient()` needs `SUPABASE_SERVICE_ROLE_KEY`; CI always provides it (workflow writes `.env.local`). If absent locally, the spec **skips the abandoned sub-assertion** (belt-and-braces) — the derivation is unit-covered (U-T4), so the gate isn't hollow. Seam is two-gated: `NODE_ENV` (weak — Playwright leaves it undefined) **and a host allow-list equality check** (`127.0.0.1`/`localhost` — the real gate); teardown-deleted.
...
- **[L] Stale-seam host gate** (sec #4): equality against `{127.0.0.1, localhost}` allow-list (NODE_ENV is the weaker gate — Playwright leaves it undefined).
